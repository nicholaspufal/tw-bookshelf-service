...
describe BookSpreadsheetFetcher do
  ...
  let(:reader) { BookSpreadsheetFetcher.new(config) }
  ...
  context "session opened" do
    ...
    it "ignores the first row and delegates to BookFactory each of the other rows" do
      ...
      BookFactory.should_receive(:generate_book).with(second_row, "poa")
      BookFactory.should_receive(:generate_book).with(third_row, "poa")
